...
const NAME_HEADERS = ["ad", "isim", "enstitü", "enstitu", "name", "enstitü adı", "enstitu adi"];
...
/**
 * Bir satırdaki hücreleri, başlık satırına göre bulunan isim sütun
 * indeksinden okur. Başlık eşleşmezse ilk sütunu (A) kullanır (bkz.
 * üniversite içe aktarma ile aynı desen).
 */
function resolveColumns(headerRow: ExcelJS.Row): { nameCol: number } {
...
export async function POST(req: Request) {
  try {
    ...
    const { nameCol } = resolveColumns(headerRow);
    ...
    for (let rowNumber = 2; rowNumber <= sheet.rowCount; rowNumber++) {
      ...
      if (!slug) {
        skipped++;
        errors.push(`Satır ${rowNumber}: geçersiz enstitü adı.`);
        continue;
      }
